...
import edu.uci.ics.fuzzyjoin.tests.dataset.PUBSmallDataset;

public class FuzzyRSJoinTest {
    private static final AbstractDataset dataset = new PUBSmallDataset(
            "../data");

    private final String[] args = new String[] {
            "-D" + FuzzyJoinDriver.DATA_DIR_PROPERTY + "=" + dataset.getPath(),
            "-D" + FuzzyJoinConfig.RECORD_DATA_PROPERTY + "=2,3",
            "-D" + FuzzyJoinConfig.RECORD_DATA1_PROPERTY + "=2,3",
            "-D" + FuzzyJoinConfig.SIMILARITY_THRESHOLD_PROPERTY + "="
                    + dataset.getThreshold(),
            "-D" + FuzzyJoinDriver.DATA_SUFFIX_INPUT_PROPERTY + "=dblp,csx" };

    @Test
    public void test010() throws Exception {
        args[args.length - 1] = "-D"
                + FuzzyJoinDriver.DATA_SUFFIX_INPUT_PROPERTY + "=dblp";
        RecordBuild.main(args);
        args[args.length - 1] = "-D"
                + FuzzyJoinDriver.DATA_SUFFIX_INPUT_PROPERTY + "=csx";
        RecordBuild.main(args);
    }
